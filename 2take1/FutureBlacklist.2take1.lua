local json = require('FBL/json')

local path_root = utils.get_appdata_path('PopstarDevs/2Take1Menu/scripts', '') .."/FBL/"
if not utils.dir_exists(path_root .. "FBL") then utils.make_dir(path_root) end
local saved_settings = IniParser("settings.ini")
...
local functions = {
	...
	api_add_player = function(rid, name, ip, reason, modder, callback)
		...
		if(tonumber(code) ~= 200) then return callback("Invalid response code: "..code) end
		local parsed = json.decode(body)
...
if not menu.is_trusted_mode_enabled(1 << 3) then
	functions.notify("You must turn on trusted mode->Http to use this script.", "error")
	return menu.exit()
end
...
menu.create_thread(function() 
	while true do
		...
		if (not network.is_session_started()) then
			if (settings.developer_mode) then functions.notify('[Dev] hopping to online mode Story > Online') end
			network.join_new_lobby(0)
			system.yield(30 * 1000)
		end
		if #script.scanned_players == 0 then
			if (settings.developer_mode) then functions.notify('[Dev] switching sessions Online > Online') end
			network.join_new_lobby(0)
